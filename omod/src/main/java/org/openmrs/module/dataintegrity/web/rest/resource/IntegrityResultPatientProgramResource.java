...
@Resource(name = RestConstants.VERSION_1  + DataIntegrityRestController.DATA_INTEGRITY_REST_NAMESPACE + "/result_by_patient_program", supportedClass = DataIntegrityResult.class, supportedOpenmrsVersions = { "1.8.*",
        "1.9.*", "1.10.*", "1.11.*", "1.12.*", "2.0.*", "2.1.*" })
public class IntegrityResultPatientProgramResource extends DelegatingCrudResource<DataIntegrityResult> {
    ...
    public PageableResult doGetAll(RequestContext requestContext) throws ResponseException {
        int programId = Integer.parseInt(requestContext.getParameter("programId"));
        return new NeedsPaging<>(getDataIntegrityService().getResultsByPatientProgram(programId), requestContext);
    }
}
